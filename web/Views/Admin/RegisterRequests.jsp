...
<html>
  ...
  <head>
    ...
    <fmt:bundle basename="i18n.Bundle"><title>Register Requests</title></fmt:bundle>
    ...
    <fmt:bundle basename="i18n.Bundle">
      <div class="main">
        ...
        <div class="Request_details popup_center">
          ...
          <c:forEach  items="${rq}" var="request" >
              ...
              <form action="" method="post" class="details_form">
                <table class="request_details_table">
                  <tbody>
                    <tr>
                      <td>FullName</td>
                      <td><span>${request.getName()}</span></td>
                    </tr>
                    <tr>
                      <td>
                        <c:if test="${request.getClientType() == 'Individual'}">Birthdate</c:if>
                        <c:if test="${request.getClientType() == 'Organisation'}">Creation date</c:if>

                        </td>
                        <td><span>${request.getBirthDate()}</span></td>
                    </tr>
                    <tr>
                      <td>Username</td>
                      <td><span>${request.getUsername()}</span></td>
                    </tr>
                    <tr>
                      <td>Email</td>
                      <td><span>${request.getEmail()}</span></td>
                    </tr>
                    <tr>
                      <td>Phone number</td>
                      <td><span>${request.getPhoneNumber()}</span></td>
                    </tr>
                    <tr>
                      <td>Nationality</td>
                      <td><span>${request.getNationality()}</span></td>
                    </tr>
                    <tr>
                      <td>Address</td>
                      <td><p>${request.getAdress()}</p></td>
                    </tr>

                    <tr>
                      <td>Client type</td>
                      <td><span>${request.getClientType()}</span></td>
                    </tr>
                    <tr>
                      <td class="documents_cell">Documents</td>
                      <td >
                        <ul id="images">
                          <c:forEach  items="${request.getFiles()}" var="file" varStatus="i">
                            <li><img src="${file}" height="100" alt="alt"/></li>
                            </c:forEach>
                        </ul>
                      </td>
                    </tr>
                    <tr>
                      <td>
                        <input type="submit" name="approuve" value="Approuve" class="approuve">
                        <input type="hidden" name="id" value="" class="hidden_input_id">
                      </td>
                      <td>
                        <input type="submit" name="reject" value="Reject" class="reject">
                        <input type="hidden" name="path" value="${request.getDocumentsPath()}" class=""></td>
                    </tr>
                  </tbody>
                </table>
                ...
                <tbody>
                  <c:forEach  items="${rq}" var="rq" >
                    ...
                    <tr>
                      ...
                      <!-- Options -->
                      <td headers="th7">
                        <button id="${rq.getId()}" onclick="showRegisterRequest(this)">
